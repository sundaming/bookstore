...
def register_handle(request):
    ...
    # 给用户的邮箱发激活邮件
    send_mail('新年发大财!', '', settings.EMAIL_FROM, [email],
              html_message='<a href="http://29.107.118.237:8888/users/active/%s/">http://127.0.0.1:8888/users/active/</a>' % token)
    send_active_email.delay(token, username, email)

    # 注册完，还是返回注册页。
    # return render(request, 'books/index.html')
    serializer = Serializer(settings.SECRET_KEY, 3600)
    token = serializer.dumps({'confirm': passport.id})  # 返回bytes2进制
    token = token.decode()  # 转译
    # send_mail('预祝您新年好!', '', settings.EMAIL_FROM, [email],
